...
select
        ...
        , null /*ct.ifct_bank_relatie*/ as "bank_naam"                          -- zib definieert de naam van de bank.
        ...
        , null                          as "begin_datumtijd_verzekering"        -- zib definieert de begin datum dat de dekking van de verzekering geldt.
        , null                          as "eind_datumtijd_verzekering"         -- zib definieert de datum tot wanneer de dekking van verzekering geldt.
        , null                          as "verzekerings_soort"                 -- zib Betaler definieert VerzekeringssoortCodes: https://zibs.nl/wiki/Betaler(NL)#VerzekeringssoortCodelijst
        , st.ifst_label_uzovi           as "zorgverzekeraar_id"                 -- zib definieert het unieke zorgverzekeraarsidentificatienummer (UZOVI).
        , st.ifst_label_naam            as "naam_zorgverzekeraar"               -- zib definieert de naam van de zorgverzekeraar, wanneer UZOVI wordt opgegeven dan moet naam vermeld worden zoals het in het UZOVI-register staat.
        , st.ifst_pat_polisnr           as "verzekerde_nummer"                  -- zib definieert de nummer van patient waarmee de persoon te herkennen is bij de zorgverzekeraar.
        , st.ifst_diag_omschrijving     as "diagnose"                           -- Reden voor behandeling. (FHIR Claim - diagnosis)
        , null                          as "behandeling"                        -- Behandeling die is ondergaan. (FHIR Claim - procedure)
...
join mtdx.v_intf_subtraject st on ct.ifct_code = st.ifst_pat_code
